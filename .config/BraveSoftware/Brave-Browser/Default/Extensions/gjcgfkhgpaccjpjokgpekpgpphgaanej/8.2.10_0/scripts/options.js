...
function generate_options_data(load_tab) {
    ...
    //show recommendations if recommendations on in settings
    if(cfg['showRecommendations']['val']) {
        cfg['moviesRecommendations']['access'] = true;
        cfg['tvSeriesRecommendations']['access'] = true;
    }
        ...
        for (var i = 0; i < conf_cat[key].length; i++) {
            ...
            switch (cfg[cfg_key]['type']) {
                ...
                case 'recommendations':
                    cfg_form_element = fillArgs('<span id="{0}-list" class="recommendations-list"></span>', cfg_key);
            }
            ...
            var cfg_reset = '';
            if (cfg[cfg_key]['type'] != 'binding' && cfg[cfg_key]['type'] != 'recommendations') {
    ...
    if (load_tab === 'tab_recommendations') {
...
function generate_options_content() {
    ...
    load_configuration(function() {
        ...
        setText('menu_settings', 'menu_settings');
        setText('menu_recommendations', 'menu_recommendations');
        ...
        document.getElementById('button_tab_settings').addEventListener('click', function() { logEvent('button_tab_settings'); tab_select('tab_settings'); });
        ...
        document.getElementById('button_tab_recommendations').addEventListener('click', function() { logEvent('button_tab_recommendations'); fill_recommendations(); tab_select('tab_recommendations'); });
...
function generate_recommendations_content(recomendations) {
    let table = document.createElement('table');
    for (let element of recomendations) {
        if (!element.item_id || !element.item_name) {
            continue;
        }
        let row = document.createElement('tr');
        let link = document.createElement('a');
        link.href = `https://www.netflix.com/watch/${ element.item_id }`;
        link.innerHTML = element.item_name;
...
function fill_recommendations() {
    let moviesRecommendationsElement = document.getElementById('moviesRecommendations-list');
    let tvSeriesRecommendationsElement = document.getElementById('tvSeriesRecommendations-list');
    if (!moviesRecommendationsElement || !tvSeriesRecommendationsElement) {
        return;
    }

    updateRecommendations();
    moviesRecommendationsElement.appendChild(
        generate_recommendations_content(recommendations.films)
    );

    tvSeriesRecommendationsElement.appendChild(
        generate_recommendations_content(recommendations.tv)
    );
}
