...
`;
    ...
    var keys = {
        ...
        'REDIRECT_SPAN': `<a href="https://svodextensions.com/hbo?utm_source=chrome_extension&utm_medium=netflix_extended&utm_campaign=gui_1" target="_blank" style="font-size:12px;color:#00B642;text-decoration:none"><span style="font-weight:bold;color:#B66D00">NEW!</span> Check Extended for HBO Max - <span style="font-weight:bold;">svodextensions.com/hbo</span></a>`,
    };

    if (['US', 'GB'].includes(getCountry())) {
        keys['REDIRECT_SPAN'] = `<a href="https://svodextensions.com/paramount?utm_source=extension&utm_medium=install&utm_campaign=netflix_extended" target="_blank" style="font-size:12px;color:#1bb3fc;text-decoration:none">🔔 Having trouble with Paramount+? Check our extensions - <b>svodextensions.com/paramount</b> </a>`
    }
    ...
    var options_data = `
        ...
        <div id="extension_options_content" style="display: none;">
            ...
            <nav class="sidebar block">
                ...
                <button class="option item menu_settings" id="button_tab_settings" style="display: none;"></button>
                ...
                <button class="option item menu_recommendations" id="button_tab_recommendations" style="display: none;"></button>
            </nav>
            ...
            <div class="content marginLeftContent marginTopContent">
                ...
                <div id="tab_about" class="tab hidden">
                    <div style="text-align: center;">
                        ...
                        <button id="button_debug" class="control button_debug hidden_class"></button>
                    </div>
                    <hr style="height: 1px;">
                    <span id="about_changelog_text" class="about_changelog_text hidden_class"></span><br>
                    <div id="about_changelog" class="about_changelog hidden_class"></div>
                </div>
...
function create_features_events() {
    try {
        document.getElementById('feature_tempHideSubtitles').addEventListener('change', function() {
            logEvent('status_updater > feature_tempHideSubtitles');
            adjust_hide_subtitles_features_values(this);
        });
    } catch (e) {}
    try {
        document.getElementById('feature_videoSpeedRate').addEventListener('input', function() {
            logEvent('status_updater > feature_videoSpeedRate');
            adjust_video_features_values(this);
        });
    } catch (e) {}
    try {
        document.getElementById('feature_videoZoom').addEventListener('input', function() {
            logEvent('status_updater > feature_videoZoom');
            adjust_video_features_values(this);
        });
    } catch (e) {}
    try {
        document.getElementById('feature_videoBrightness').addEventListener('input', function() {
            logEvent('status_updater > feature_videoBrightness');
            adjust_video_features_values(this);
        });
    } catch (e) {}
    try {
        document.getElementById('feature_videoContrast').addEventListener('input', function() {
            logEvent('status_updater > feature_videoContrast');
            adjust_video_features_values(this);
        });
    } catch (e) {}
    try {
        document.getElementById('feature_videoGrayscale').addEventListener('input', function() {
            logEvent('status_updater > feature_videoGrayscale');
            adjust_video_features_values(this);
        });
    } catch (e) {}
    try {
        document.getElementById('feature_videoHue').addEventListener('input', function() {
            logEvent('status_updater > feature_videoHue');
            adjust_video_features_values(this);
        });
    } catch (e) {}
    try {
        document.getElementById('feature_videoInvert').addEventListener('input', function() {
            logEvent('status_updater > feature_videoInvert');
            adjust_video_features_values(this);
        });
    } catch (e) {}
    try {
        document.getElementById('feature_videoSaturation').addEventListener('input', function() {
            logEvent('status_updater > feature_videoSaturation');
            adjust_video_features_values(this);
        });
    } catch (e) {}
    try {
        document.getElementById('feature_videoSepia').addEventListener('input', function() {
            logEvent('status_updater > feature_videoSepia');
            adjust_video_features_values(this);
        });
    } catch (e) {}
    var evt_elms = document.getElementsByClassName('extension_feature_reset');
    for (var i = 0; i < evt_elms.length; i++) {
        evt_elms[i].addEventListener('click', function() {
            logEvent('status_updater > extension_feature_reset');
            reset_feature_value(this.value);
        });
    }
}
    ...
    if (check_watch() && cfg['hideStatusIcon']['val'] && cfg['hideStatusIcon']['access']) {
        ...
        if (obj['bubblePlaceBeforeElm']) {
            ...
            if (!bubble_container) {
                ...
                bubble_container.addEventListener('mouseenter', function() {
                    logEvent('create_status_objects > netflex_bubble_container > enter');
                    stop_worker('close_status_content');
                    switch_simulation(true, 'netflex_bubble_container');
                });
                bubble_container.addEventListener('mouseleave', function() {
                    logEvent('create_status_objects > netflex_bubble_container > leave');
                    workers['close_status_content'] = setTimeout(function() { switch_simulation(false, 'netflex_bubble_container'); }, cfg['bubbleHideDelay']['val']);
                });

                // Set events for status elements
                document.getElementById('extension_news').addEventListener('click', function() {
                    logEvent('create_status_objects > extension_news');
                    extension_news();
                });

                document.getElementById('extension_options').addEventListener('click', function() {
                    logEvent('create_status_objects > extension_options');
                    extension_options();
                });
                document.getElementById('extension_options').addEventListener('mouseenter', function() {
                    logEvent('create_status_objects > extension_options > enter');
                    options_icon_animate(true);
                });
                document.getElementById('extension_options').addEventListener('mouseleave', function() {
                    logEvent('create_status_objects > extension_options > leave');
                    options_icon_animate(false);
                });

                document.getElementById('extension_features').addEventListener('click', function() {
                    logEvent('create_status_objects > extension_features');
                    extension_features();
                });

                document.getElementById('extension_logo').addEventListener('click', function() {
                    if (check_watch()) {
                        logEvent('create_status_objects > extension_logo');
                        create_fireworks_mark();
                        stop_worker('close_status_content');
                        switch_simulation(false, 'netflex_bubble_container');
                    }
                });
        ...
        if (obj['iconPlaceBeforeElm']) {
            ...
            if (obj['iconPlaceBeforeElm'] && obj['iconTemplateElm'] && !icon_container) {
                ...
                icon_container.addEventListener('mouseenter', function() {
                    logEvent('create_status_objects > netflex_icon_container > enter');
                    stop_worker('close_status_content');
                    switch_simulation(true, 'netflex_bubble_container');
                });
                icon_container.addEventListener('mouseleave', function() {
                    logEvent('create_status_objects > netflex_icon_container > leave');
                    workers['close_status_content'] = setTimeout(function() { switch_simulation(false, 'netflex_bubble_container'); }, cfg['bubbleHideDelay']['val']);
                });
                    ...
                    for (var i = 0; i < elms.length; i++) {
                        ...
                        if (elm_id != 'netflex_icon_container') {
                            ...
                            if (elms[i].id !== 'extension_status_content' && elms[i].id !== 'extension_status_button' && attr !== 'true') {
                                elms[i].addEventListener('mouseenter', function() {
                                    logEvent('status_updater > player_controls_elements/navigation_menu_elements');
                                    stop_worker('close_status_content');
                                    switch_simulation(false, 'netflex_bubble_container');
                                });
                ...
                if (icon_container) {
                    ...
                    if (icon_element) {
                        if (!icon_element.getAttribute('events')) {
                            icon_element.addEventListener('click', function(e) {
                                if (e.button == 0) {
                                    logEvent('create_status_objects > extension_status');
                                    toggle_assistant();
                                }
                            });
